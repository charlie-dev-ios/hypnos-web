...
- とくいきのみが複数存在するフィールド（ランダム選択）の場合、候補となるきのみの情報が分かること
...
- **FR-004**: システムはカビゴン評価の各ランクごとに、そのフィールドで出現するポケモンの一覧を表示しなければならない
- **FR-005**: システムはフィールド一覧を島ガイドページ上に表示し、各フィールドの詳細情報へ遷移できなければならない
...
- フィールド数は現時点でゲーム内に存在するフィールド（ワカクサ本島、シアンの砂浜、トープ洞窟、ウノハナ雪原、ラピスラズリ湖畔 等）を対象とする。将来のフィールド追加にも対応できるデータ構造とする
